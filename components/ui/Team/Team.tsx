...
export default async function ourTeam() {
    ...
    return(
        <section>
            <a href="#team"></a>
            <div className={s.teamsectioncontainer}>
                ...
                <div className={s.teamcontainer}>
                    {team.teamMembers.map((member, index) => {
                        ...
                        <div className="text-center">
                            <div className="w-54 h-54 relative text-light-secondary dark:text-dark-secondary flex items-center justify-center">
                                ...
                                <img className="z-10 absolute w-36 h-36 rounded-full hover:shadow-center" src={member.portrait} alt={member.name} />
                            </div>
                            <h3>
                                {member.name}
                            </h3>
                            ...
                            <ul className="flex justify-center mt-4 space-x-4">
                                {Object.entries(member.socialProfiles).map(([key, value]) => {
                                    return (
                                        <SocialIcon iconName={key} url={value} className={s.svgicons}/>
                                    );
                                })}
                            </ul>
                        </div>
